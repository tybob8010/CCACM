    ...
    const CCACM = {
        ...
        init: function() {
            ...
            style.innerHTML = `
                ...
                .ccacm-base { position: absolute !important; bottom: 50px !important; right: 5px; width: 48px; height: 48px; z-index: 1000000; pointer-events: none; }
                .ccacm-icon-shaker { position: absolute; width: 48px; height: 48px; z-index: 10; animation: ccacmX_Extreme 0.6s infinite alternate ease-in-out, ccacmY_Extreme 0.3s infinite alternate ease-in-out; pointer-events: none; }
                #ccacm_icon { width: 48px !important; height: 48px !important; background: url(img/icons.png) ${-4 * 48}px ${-0 * 48}px !important; cursor: pointer !important; filter: drop-shadow(0px 0px 4px #000) !important; position: relative; z-index: 20; pointer-events: auto; transition: filter 0.1s ease-out; }
                #ccacm_icon:hover { filter: drop-shadow(0px 0px 6px rgba(255,255,255,0.7)) brightness(1.0) !important; }
                #ccacm_shine { position: absolute; width: 60px; height: 60px; top: -10px; left: -5px; background: url(img/shine.png) no-repeat center; background-size: contain; z-index: 1; opacity: 0; animation: ccacmRotate 20s infinite linear; pointer-events: none; }
                .ccacm-base:has(#ccacm_icon:hover) #ccacm_shine { opacity: 0.6; transition: opacity 0.3s ease-out; }
            `;
            ...
            setInterval(() => {
                ...
                if (this.config.lastExecutedDay === todayStr && this.config.lastExecutedTime === this.config.targetTime) return;
        ...
        prepareIcon: function() {
            ...
            if (target) {
                ...
                icon.id = 'ccacm_icon';
                icon.onmouseover = () => { Game.tooltip.draw(icon, '<div style="padding:8px;width:180px;text-align:center;"><b>CCACM 設定</b><br>クリックで設定画面を開く</div>', 'this'); };
